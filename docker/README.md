...
| Сервис | URL (снаружи) | URL (внутри Docker) | Описание |
|--------|---------------|---------------------|----------|
| **Web приложение** | http://localhost:5000 | http://web:8080 | Blazor интерфейс |
| **API** | http://localhost:5001 | http://api:8080 | REST API |
| **Swagger** | http://localhost:5001/swagger | http://api:8080/swagger | API документация |
| **PostgreSQL** | localhost:5432 | postgres:5432 | База данных с pgvector |
| **Ollama** | http://localhost:11434 | http://ollama:11434 | LLM API |

**Важно:** Контейнеры внутри Docker-сети взаимодействуют друг с другом по именам сервисов (api, web, postgres, ollama), а не через localhost.
...
### Настройки подключения веб-приложения к API

Веб-приложение использует следующие переменные окружения для подключения к API:

```yaml
ApiSettings__BaseUrl=http://api:8080/
```

**Важно:** Внутри Docker-сети используется имя сервиса `api` вместо `localhost`, потому что:
- Контейнеры изолированы друг от друга
- `localhost` в контейнере указывает на сам контейнер
- Имена сервисов из docker-compose.yml используются как DNS-имена в Docker-сети

### CORS настройки

API настроен для приема запросов от:
- `http://localhost:5000` - для доступа снаружи Docker
- `http://web:8080` - для доступа изнутри Docker-сети
